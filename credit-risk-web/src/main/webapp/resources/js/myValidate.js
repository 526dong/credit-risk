...
function myValidateEasy(obj){
    ...
    for (var i = 0; i < nameArr.length; i++) {
        var name = nameArr[i];
        var inputLabel = $("input[name='"+name+"']");
        var rule = ruleArr[i];

        if (inputLabel.size() == 1) {
            pass = showError(pass, rule, inputLabel);
        } else if (inputLabel.size() > 1)  {
            $(inputLabel).each(function () {
                pass = showError(pass, rule, $(this));
            });
        }
    }
 ...
 * @param inputVal
 */
function showError(pass, rule, inputLabel) {
    //规则为数组
    if (rule instanceof Array) {
        for (var j = 0; j < rule.length; j++) {
            if (!valRule(rule[j], inputLabel)) {
                pass = false;
                inputLabel.next().show();
                break;
            }
        }
    } else {
        //单个规则
        if (!valRule(rule, inputLabel)) {
            pass = false;
            inputLabel.next().show();
        }
    }
