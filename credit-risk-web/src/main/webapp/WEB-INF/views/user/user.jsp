...
//         if (data[i].status == 0) {
//         	htmlContent += "<shiro:hasPermission name='/user/freeze'>";
//         	htmlContent += "<a href='javaScript:;' onclick=freeze('"+data[i].id+"','"+data[i].loginName+"') title='停用' class='operate_a12 left72' ></a>";
//         	htmlContent += "</shiro:hasPermission>";
//         } if(data[i].status == 1){
//         	htmlContent += "<shiro:hasPermission name='/user/unfreeze'>";
//         	htmlContent += "<a href='javaScript:;' onclick=unfreeze('"+data[i].id+"','"+data[i].loginName+"') title='启用' class='operate_a14 left72' ></a>";
...
//         }if(data[i].status == 2){
//         	htmlContent += "<shiro:hasPermission name='/user/unlock'>";
//         	htmlContent += "<a href='javaScript:;' onclick=unlock('"+data[i].id+"','"+data[i].loginName+"') title='解锁' class='operate_a13 left104' ></a>";
//         	htmlContent += "</shiro:hasPermission>";
//         }
//         htmlContent += "<shiro:hasPermission name='/user/isDel'>";
//         htmlContent += "<a href='javaScript:;' onclick=isDel('"+data[i].id+"','"+data[i].loginName+"') title='删除' class='operate_a3' ></a>";
//         htmlContent += "</shiro:hasPermission>";
        htmlContent += "</td>";
        htmlContent += "</tr>";
    }
...
function addUser(){
	$.ajax({
        url : "${ctx}/user/checkUserNumLimit",
        type : 'POST',
        async : false,
        success : function(data) {
            if(null != data && null != data.msg && ""!=data.msg){
            	if("0000" == data.msg){
            		...
            		var userNumLimit = data.userNumLimit;
            		alert("现有用户数量已经达到所设置的用户数量上限，请联系后台管理员更改上限后再创建用户");
            	}
            }
        }
    });
            ...
            <div class="module_box">
                <div class="module_search">
                    ...
                    <div class="fr assets_btn">
                        <a href="javaScript:addUser();" class="newAddBody role_btn">用户创建</a>
                    </div>
                    </shiro:hasPermission>
                </div>
