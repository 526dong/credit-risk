...
import com.ccx.credit.risk.mapper.RoleMapper;
...
public class RoleServiceImpl extends ServiceImpl<RoleMapper, Role> implements RoleApi {
	...
	@Override
	public List<Map<String, Object>> findRoleByInstitutionId(long id) {
		List<Map<String, Object>> listMap=roleMapper.findRoleByInstitutionId(id);
		return listMap;
	}
	...
	public JSONArray treeData(HttpServletRequest request, List<RoleResource> roleRes) {
		...
		for (PermissionBean resource : list) {
			...
			obj.put("name", resource.getPermission_name());
	...
	public JSONArray showTree(List<RoleResource> roleRes,long insId) {
		...
		for (PermissionBean resource : pList) {
			...
			obje.put("name", resource.getPermission_name());
			if (!list.isEmpty()) {
