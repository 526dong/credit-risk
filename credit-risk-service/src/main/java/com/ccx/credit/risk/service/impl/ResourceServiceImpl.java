...
import com.ccx.credit.risk.mapper.ResourceMapper;
...
import com.ccx.credit.risk.model.PermissionTreeBeanFg;
...
//	}
	 ...
	 */
	@Override
	public String finRolePermissionTree(Long id,Long insId){
		//根据机构查询所拥有的模块		
		List<Module> moduleList = userMapper.getModuleByInsId(insId);
//		List<PermissionTreeBeanFg> list = resourceMapper.finRolePermissionTree(id);
		List<PermissionTreeBeanFg> list = new ArrayList<PermissionTreeBeanFg>();
		String myselfId = "manage-0";
		List<PermissionTreeBeanFg> list1 = resourceMapper.findInsPermissionTreeMy(id,myselfId);
		list.addAll(list1);
		if(null != moduleList && moduleList.size()>0){
			for (int i = 0; i < moduleList.size(); i++) {
				myselfId = moduleList.get(i).getMyselfId();
				list1 = resourceMapper.findInsPermissionTreeMy(id,myselfId);
				list.addAll(list1);
			}
		}
		String result = "";
